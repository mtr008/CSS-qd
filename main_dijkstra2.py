...
def improved_css_img(X, K, max_iter, lmda_1):
    ...
    V = list(range(h * w))
    Graph_X = nx.Graph()
    Graph_X = add_vertices_and_edges(Graph_X, h, w, V)  # 2
    ...
    lab_1 = np.mean(aug_X[1:h - 1, 1:w - 1, :3] + aug_X[:h - 2, :w - 2, :3] +
             aug_X[:h - 2, 1:w - 1, :3] + aug_X[1:h - 1, :w - 2, :3],axis=-1)
    lab_2 = np.mean(aug_X[1:h - 1, 1:w - 1, :3] + aug_X[1:h - 1, :w - 2, :3] +
             aug_X[2:h, :w - 2, :3] + aug_X[2:h, 1:w - 1, :3],axis=-1)
    lab_3 = np.mean(aug_X[1:h - 1, 1:w - 1, :3] + aug_X[2:h, 1:w - 1, :3] +
             aug_X[1:h - 1, 2:w, :3] + aug_X[2:h, 2:w, :3],axis=-1)
    lab_4 = np.mean(aug_X[1:h - 1, 1:w - 1, :3] + aug_X[:h - 2, 1:w - 1, :3] +
             aug_X[1:h - 1, 2:w, :3] + aug_X[:h - 2, 2:w, :3],axis=-1)
    ...
    c_point_init = V[np.random.randint(0, h * w - 1)]
    C_set = []
    ...
    while delta < 0 and iter <= max_iter:  # 10
        ...
        [L, min_dist] = label(aug_X, min_dist, L, S, K, C_set, Area)
...
def label(aug_im, min_dist, L, S, num_clusters, C_set, Area):
    h = aug_im.shape[0]
    w = aug_im.shape[1]
    ...
    for k in range(num_clusters):
        ...
        dist = np.sqrt(np.sum(np.square(im_patch - mu_k), axis=-1))
...
# tt = manifold_slic(im, 10, [1, 2, 3])

num_clusters = [25]
for num_clusters in num_clusters:
    [src_set, out_im, cluster_centers] = improved_css_img(X=im, K=num_clusters, max_iter=10, lmda_1=1)
    imsave(fn('outputs/dijkstra_' + str(num_clusters) + '_centers.jpg'),
           normalize_im(create_centers_im(im.copy(), cluster_centers)))
    # out_im = manifold_slic(im, num_clusters, cluster_centers)
    ...
    imsave(fn('outputs/dijkstra_' + str(num_clusters) + '.jpg'), normalize_im(dimg))
